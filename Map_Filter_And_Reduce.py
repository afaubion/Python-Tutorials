...
# in one full swoop. map and filter come built-in with Python (in the __builtins__ module)
# and require no importing.
...
# It means there can be as many iterables as possible, in so far func has that exact number
...
# Note that using the defined map() syntax above, func in this case is str.upper
# and iterables is the my_pets list -- just one iterable.
...
# the first element of circle_areas, 3.56773 is passed along with the first element of range(1,7),
# 1 to round, making it effectively become round(3.56773, 1).

# During the second iteration, the second element of circle_areas, 5.57668 along with
# the second element of range(1,7), 2 is passed to round making it translate to round(5.57668, 2).
# This happens until the end of the circle_areas list is reached.

# I'm sure you're wondering: "What if I pass in an iterable less than or more than the length
# of the first iterable? That is, what if I pass range(1,3) or range(1, 9999) as the
# second iterable in the above function".
...
# it will simply iterate over the elements until it can't find a second argument
# to the function, at which point it simply stops and returns the result.
...
# Instead, this is what Python does: It takes the first element of circle_areas
# and the first element of range(1,3) and passes it to round.
# round evaluates it then saves the result.
# Then it goes on to the second iteration, second element of circle_areas
# and second element of range(1,3), round saves it again.
...
# our own custom zip() function. The zip() function is a function that takes a number of iterables
# and then creates a tuple containing each of the elements in the iterables.
# Like map(), in Python 3, it returns a generator object, which can be easily converted
# to a list by calling the built-in list function on it.
...
# Did you also notice that I didn't even need to create a function
# using the def my_function() standard way?
# That's how flexible map(), and Python in general, is!
# I simply used a lambda function.
# This is not to say that using the standard function definition method (of def function_name())
# isn't allowed, it still is. I simply preferred to write less code (be "Pythonic").

# That's all about map. Onto filter()

# Filter
# While map() passes each element in the iterable through a function and returns
# the result of all elements having passed through the function, filter(),
# first of all, requires the function to return boolean values (true or false)
...
#       (if initial is not supplied) and the second the second element in iterable.
...
# As usual, it's all about iterations: reduce takes the first and second elements in numbers
# and passes them to custom_sum respectively.
...
# The result, as you'll expect, is 78 because reduce, initially,
