...
# it will use the function draw_game from draw.py, or in other words, the draw module
# that implements the logic for drawing the game on screen.
...
# to recap:
# in this example, the game module imports the draw module, which enables it to
# use functions implemented in that module

# the main function would use the local function play_game to run the game, and then run the results
# of the game using the function implemented in the draw module called draw_game.

# to use the function draw_game from the draw module, we would need to specify in which module the
# function is implemented, using the dot operator.

# To reference the draw_game function from the game module, we would need
# to import the draw module and only then call draw.draw_game().

# When the import draw directive will run, the Python interpreter will look for a file
# in the directory which the script was executed from, by the name of the module with a .py prefix,
# so in our case it will try to look for draw.py. If it will find one, it will import it.
...
# Python compiles files into Python bytecode so that it won't have to parse the files
# each time modules are loaded.
...
# into the main script's namespace, by using the from command:
"""
# game.py
# import the draw module
...
# You may have noticed that in this example, draw_game does not precede
# with the name of the module it is imported from,
...
# inside the current module because you don't need to specify which module the function comes from.
...
# This is useful to know, because this means that you can rely
# on this behavior for initializing objects. For example:
"""
# draw.py
...
# You could either use the environment variable PYTHONPATH to specify additional directories
# to look for modules in, like this:
"""
PYTHONPATH=/foo python game.py
"""
# This will execute game.py, and will enable the script to load modules from the foo directory
...
# Link to list of built in standard python libraries: https://docs.python.org/3/library/
...
# a module inside that package called bar.
